...
import { BedDouble, Filter, LogOut, RefreshCw, Search, UserRound, X } from 'lucide-react';

const STATUS_COLORS = { Available: '#22c55e', Occupied: '#ef4444', Cleaning: '#8b5cf6' };
const STATUSES = ['Available', 'Occupied', 'Cleaning'];
const BED_TYPES = ['All Types', 'General', 'ICU', 'Private', 'Emergency', 'Maternity', 'Pediatric'];
const TABS = [
...
export default function NursePortal() {
  ...
  const loadBeds = useCallback(() => {
    api.get('/beds', { params: { hospitalId: user?.hospitalId } })
  ...
  useEffect(() => {
    ...
    // Server-emit socket connection for bed updates
    socket.connect();
    ...
    api.get(`/nurses?hospitalId=${user?.hospitalId}`).then((res) => {
      ...
      const record = data.find((item) => item.nurseId === nurseId);
    ...
    socket.on('bed:update', (updatedBed) => {
      setBeds((prev) => prev.map((item) => (item.bedId === updatedBed.bedId ? { ...item, status: updatedBed.status } : item)));
    });
  ...
  const wards = [...new Set(beds.map((bed) => bed.ward))].sort();
  ...
  const filtered = beds.filter((bed) => {
    const matchSearch = !search ||
      ...
      (bed.ward || '').includes(search);
    const matchStatus = !filterStatus || bed.status === filterStatus;
    const matchWard = !filterWard || bed.ward === filterWard;
  ...
  const updateStatus = async (bedId, status) => {
    try {
      ...
      loadBeds(); // Refresh to get correct server state
    } catch (err) {
  ...
  beds.forEach((bed) => { if (stats[bed.status] !== undefined) stats[bed.status] += 1; });
        ...
        <div className="container" style={{ maxWidth: 1120 }}>
          <section className="portal-header-card">
            <div className="portal-header-copy">
              ...
              <p>Fast updates for occupancy and cleaning workflows to keep hospital availability live and accurate.</p>
              ...
              <div className="card" style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  <div style={{ position: 'relative', flex: 1 }}>
                    ...
                    <input className="form-input" style={{ paddingLeft: 32 }} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by bed or ward" />
                  </div>
                  <button className="btn btn-ghost btn-sm" onClick={() => setShowFilters((prev) => !prev)}><Filter size={14} /> Filters</button>
                  ...
                  <div className="bed-grid">
                    {filtered.map((bed) => (
                      <div key={bed.bedId} className={`bed-card ${bed.status}`} onClick={() => setSelected(bed)}>
                        <div className="bed-num">{bed.bedId.split('-').pop()}</div>
                        <div className="bed-ward">Ward {bed.ward || 'General'}</div>
                        <div className="bed-type">{bed.bedType}</div>
                        {bed.patientName && <div style={{ marginTop: 4, fontSize: 11, fontWeight: 700 }}>{bed.patientName}</div>}
                        ...
                        {bed.updatedAt && <div style={{ marginTop: 6, fontSize: 10, color: 'var(--text3)' }}>Updated {timeSince(bed.updatedAt)}</div>}
                      </div>
                    ))}
                  </div>
                ) : (
                    ...
                    {filtered.map((bed) => (
                      <div key={bed.bedId} className="card" style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', borderLeft: `3px solid ${STATUS_COLORS[bed.status]}` }} onClick={() => setSelected(bed)}>
                        <div style={{ flex: 1 }}>
                          ...
                          <div className="text-muted">Ward {bed.ward || 'General'} | {bed.bedType}</div>
                          {bed.patientName && <div style={{ marginTop: 3, fontSize: 12, fontWeight: 600 }}>{bed.patientName}</div>}
                        </div>
                ...
                {!loading && filtered.length === 0 && <div className="empty"><p>No beds match the current filters.</p></div>}
              </div>
            </div>
          )}
          ...
          {tab === 'profile' && (
            <div className="card">
              <div style={{ textAlign: 'center', padding: '10px 0 16px' }}>
                ...
                <div className="text-muted" style={{ marginTop: 4 }}>{nurse?.contact || '-'}</div>
        ...
        <div className="modal-overlay" onClick={() => { setSelected(null); setPatientName(''); }}>
          <div className="modal" onClick={(event) => event.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">{selected.bedId}</span>
            ...
            {selected.patientName && <p style={{ marginBottom: 10, fontWeight: 600 }}>Patient: {selected.patientName}</p>}
            <div className="form-group">
              <label className="form-label">Patient name (if occupying)</label>
